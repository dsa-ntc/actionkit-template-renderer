...
def user(
    recurring=1, payment_hash=False, customfields=None, previous_recurring=False, id=666
):
    if not customfields:
        customfields = {
            "occupation": "witch",
...
def order(
    order_type="order", details=None, quickpay=False, paypal=False, apple=False
):  # get rid of paypal and applepay and switch to "payment method"
...
contexts = {
    ...
    "donate.4": compose(
        [base("suggested_ask, recurring")], ["suggested_ask", "donation_type"]
    ),
    "donate.5": compose(
        [base("other suggested_ask, single")], ["suggested_ask", "donation_type"], -1
    ),
    "donate.6": compose(
        [base("suggested_ask, payment_hash"), user(0, payment_hash=True)],
        ["suggested_ask", "payment_hash"],
    ),
    "donate.7": compose([base("user"), user()]),
    "donate.8": compose([base("pac", entity="pac", show_paypal=True), user(id=507809)]),
    "donate.9": compose(
        [
            base("candidate suggested", entity="pac", layout="donate_5050_split"),
            candidates,
        ],
        ["suggested_ask"],
        -1,
    ...
    "donate.10": compose(
        [
            base("two candidates suggested", entity="pac", layout="donate_5050_split"),
            user(id=507809),
            candidates2,
        ],
        ["suggested_ask"],
    ...
    "donate.11": compose(
        [base("two candidates suggested no 5050", entity="pac"), candidates2],
        ["suggested_ask"],
        -1,
    ),
    "donate.12": compose(
        [
            base("candidate, quickpay", entity="pac", layout="donate_5050_split"),
    ...
    "donate.13": compose(
        [
            base("two candidates quickpay", entity="pac", layout="donate_5050_split"),
            user(0, payment_hash=True),
            candidates2,
        ],
        ["payment_hash"],
    ...
    "donate.16": compose(
        [base("weekly recurring checkbox", layout="make_weekly_checkbox")], ["weekly"]
    ),
    "donate.17": compose(
        [
            base(
                "quickpay recurring checkbox", entity="pac", layout="donate_5050_split"
            ),
            user(0, payment_hash=True),
            candidates,
        ],
    ...
    "donate.18": compose(
        [
            base(
                "quickpay",
                entity="pac",
                layout="donate_5050_split donation_no_checkbox",
            ),
            user(0, payment_hash=True),
            candidates,
        ],
    ...
    "donate.19": compose(
        [
            base("quickpay with weekly", entity="pac", layout="make_weekly_checkbox"),
            user(0, payment_hash=True),
            candidates,
        ],
        ["payment_hash"],
    ...
    "donate.20": compose(
        [
            base("quickpay", entity="pac", layout="weekly_only"),
            user(0, payment_hash=True),
            candidates,
        ],
        ["payment_hash"],
    ...
    "donate.22": compose(
        [base("quickpay with monthly recurring"), user(0, payment_hash=True)],
        ["donation_type", "payment_hash"],
    ),
    ...
    "donate.26": compose(
        [
            base("weekly and two candidates", entity="pac", layout="weekly_only"),
            candidates2,
        ]
    ),
    ...
    "donate.28": compose(
        [
            base("pac", entity="pac", show_paypal=True, layout="accept_ach"),
            user(id=507809),
        ]
    ),
    "donate.29": compose(
        [base("pac", entity="pac", show_paypal=True), user(id=507810)]
    ),
    "donate.30": compose(
        [base("pac with ach option", entity="pac", accept_ach=True), user(id=507809)]
    ),
    "donate.31": compose(
        [
            base(
                "quickpay with a good weekly param combination",
                entity="pac",
                layout="weekly_only",
            ),
            user(0, payment_hash=True),
    ...
    "donate.32": compose(
        [
            base(
                "quickpay with a bad weekly param combination",
                entity="pac",
                layout="weekly_only",
            ),
            user(0, payment_hash=True),
    ...
    "donate.thanks.1": compose(
        [
            base("civ with payment_hash", filename="thanks.html"),
            user(0, payment_hash=True),
            order(),
        ]
    ),
    "donate.thanks.2": compose(
        [
            base("recurring civ", entity="pac", filename="thanks.html"),
            user(),
    ...
    "donate.thanks.3": compose(
        [
            base(
                "recurring civ receipt message",
                entity="pac",
                filename="thanks.html",
                fields={
                    "thanks_below_receipt_message": "<b>This message is below the receipt!</b>"
                },
            ),
            user(),
            order("orderrecurring"),
    ...
    "donate.thanks.4": compose(
        [
            base("recurring user recurring", filename="thanks.html"),
            user(2),
            order("orderrecurring"),
        ]
    ),
    "donate.thanks.5": compose(
        [
            base("pac user payment_hash", entity="pac", filename="thanks.html"),
            user(0, payment_hash=True),
            order(),
        ]
    ),
    "donate.thanks.6": compose(
        [
            base("pac user quickpay no employer", entity="pac", filename="thanks.html"),
            user(0, payment_hash=True, customfields={"x": 1}),
            order(quickpay=True),
        ]
    ),
    "donate.thanks.7": compose(
        [
            base("pac user candidate", entity="pac", filename="thanks.html"),
            user(0, payment_hash=True),
            order(
                details={
                    "order_details": [
                        {"candidate": {"name": "Francis Underwood"}, "amount": 10.00},
                    ],
                    "other_amount": 2.07,
                }
            ),
    ...
    "donate.thanks.9": compose(
        [
            base("civ with payment_hash", filename="thanks.html"),
            user(recurring=0, payment_hash=True, previous_recurring=False),
            order(paypal=True),
        ]
    ),
    "donate.thanks.10": compose(
        [
            base("civ with payment_hash", filename="thanks.html"),
            user(recurring=1, payment_hash=True, previous_recurring=True),
            order(paypal=True),
        ]
    ),
    "donate.thanks.11": compose(
        [
            base("civ with payment_hash", filename="thanks.html"),
            user(recurring=0, payment_hash=True, previous_recurring=False),
            order(),
    ...
    "donate.thanks.13": compose(
        [
            base("civ with payment_hash", filename="thanks.html"),
            user(recurring=1, payment_hash=True, previous_recurring=False),
            order(apple=True),
        ]
    ),
    "donate.thanks.14": compose(
        [
            base("civ with payment_hash", filename="thanks.html"),
            user(recurring=1, payment_hash=True, previous_recurring=True),
            order(apple=True),
