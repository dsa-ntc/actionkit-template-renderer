...
def mo_event_data(event):
    eobj = event.get("obj")
    machine_offset = time.time() - time.mktime(
        time.gmtime()
    )  # correction factor for when this runs on machines that aren't defaulting to UTC
    naive_start_time = int(time.mktime(eobj["starts_at_utc"].timetuple()))
    starts_utc = naive_start_time + machine_offset
    timezone_offset = (
        int(time.mktime(eobj["starts_at"].timetuple())) - naive_start_time
    ) / 3600
    full = (
        0
        if not event["max_attendees"]
        else int(bool(event["max_attendees"] <= event["attendee_count"]))
    )
    return {
